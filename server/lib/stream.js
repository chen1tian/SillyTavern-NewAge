...
import * as ss from '../node_modules/@sap_oss/node-socketio-stream/index.js';
  ...
  io.of(namespace).on('connection', Socket => {
    ...
    Socket.on(STREAM_EVENTS.START, data => {
      if (data.type === MSG_TYPE.STREAM_START) {
        ...
        const clientStream = forwardStreamData(io, namespace.LLM, llmRequests, data); // 调用 forwardingHandler, 获取流
        if (clientStream) {
